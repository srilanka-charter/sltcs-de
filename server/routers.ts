  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background: #f9f9f9; padding: 24px; border-radius: 8px;">
      <div style="background: #1a1a1a; color: #c9a84c; padding: 16px 24px; border-radius: 6px 6px 0 0; text-align: center;">
        <h1 style="margin: 0; font-size: 1.4rem; letter-spacing: 0.05em;">SLTCS — New Enquiry Received (DE)</h1>
    ...
    submit: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        try {
          ...
          console.log(`[Enquiry] Email sent to customer: ${input.email} (CC: srilanka.41032@gmail.com, BCC: contact@gohellolanka.com)`);
        } catch (err) {
        ...
        await notifyOwner({
          title: `[DE] New Enquiry from ${input.name} (${countryDisplay})`,
          content: notifContent,
        }).catch((e: unknown) => console.warn("[Enquiry] Manus notification failed:", e));
